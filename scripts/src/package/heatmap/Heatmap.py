...
class Heatmap : 
    def __init__(self,
        ...
        # TODO change the openfile function
        # Open files ============================================================
        OPENPATH  = "data/preprocessed/"
        df_plot_per_revue = pd.read_csv(
            OPENPATH + filename_open["open_per_revue"]
        )
        df_plot_per_revue = df_plot_per_revue.loc[
            (df_plot_per_revue["RA"] == False) &
            (df_plot_per_revue["definition"] == "restr"),
            ["annee","revue","discipline","proportion"]
        ]
        df_plot_per_revue.index = range(len(df_plot_per_revue))

        df_plot_per_discipline = pd.read_csv(
            OPENPATH + filename_open["open_per_discipline"]
        )
        df_plot_per_discipline = df_plot_per_discipline.loc[
            (df_plot_per_discipline["RA"] == True) & 
            (df_plot_per_discipline["definition"] == "restr"),
